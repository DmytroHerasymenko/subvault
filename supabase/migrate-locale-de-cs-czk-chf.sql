-- Add German/Czech locales and CZK/CHF currencies (run once in Supabase SQL Editor)

alter table public.profiles drop constraint if exists profiles_locale_check;
alter table public.profiles
  add constraint profiles_locale_check check (locale in ('ua', 'en', 'pl', 'de', 'cs'));
...
alter table public.profiles
  ...
  check (preferred_currency in ('UAH', 'USD', 'EUR', 'PLN', 'CZK', 'CHF'));
...
alter table public.subscriptions
  ...
  check (currency in ('UAH', 'USD', 'EUR', 'PLN', 'CZK', 'CHF'));
